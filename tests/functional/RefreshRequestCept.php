...
$I->wantTo('Use a Refresh token to get a new AuthToken via GraphQL mutation');
...
/**
 * Get the refresh token from the response. Typically, on initial login, this token will get stored somewhere for later
 * use so that logging in with username and password isn't necessary often. Of course, this token is revokeable, so
 * if it is used at any point to retrieve a new authToken, and it is invalid, then the user needs to re-authenticate
 * with username/password, if they haven't been revoked.
 */
